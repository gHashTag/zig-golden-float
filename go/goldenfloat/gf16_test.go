...
func testConversions(t *testing.T) bool {
	...
	for _, test := range conversions {
		...
		// Parse input — vectors.json has string for special values, number otherwise
		var inputStr string
		...
		case string:
			...
			case "nan":
				inputVal = math.NaN()
			default:
				t.Fatalf("unknown string input: %s", inputStr)
			}
		case float64:
			inputVal = v
		default:
			t.Fatalf("invalid input type for %s", name)
		}
		...
		if predicate, ok := tc["predicate"]; ok {
			...
			if matchType == "roundtrip" {
				if inputStr == "inf" || inputStr == "-inf" || inputStr == "nan" {
					...
					// Allow 1% tolerance
					backF64 := float64(back)
					if inputVal == 0 {
						result = back == 0 || backF64 == inputVal
					} else {
						relError := (backF64 - inputVal) / inputVal
	...
	for _, test := range predicates {
		...
		case string:
			...
			case "nan":
				inputVal = math.NaN()
			default:
				t.Fatalf("unknown string input: %s", v)
			}
		case float64:
			inputVal = v
		default:
